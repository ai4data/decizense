#!/usr/bin/env bash
#
# Phase 2b runtime verification — OPA authoritative (cutover).
#
# Covers:
#   - OPA sidecar up with bundle loaded (health check)
#   - Harness boots with OPA authoritative (no OPA_SHADOW, no in-code rules)
#   - Phase 0/1a regression (test-query, test-auth) still green
#   - test-opa-equivalence.ts 28 cases all pass (OPA is the sole engine)
#   - Negative: harness REFUSES to start when OPA is down
#   - GovernanceResult now includes policy_version field
#
# Usage:
#   scripts/verify-phase-2b.sh

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TIMESTAMP="$(date +%Y-%m-%dT%H-%M-%S)"
EVIDENCE_DIR="${REPO_ROOT}/docs/phase-2b-verification/${TIMESTAMP}"
...
# ── 1. Environment ──
{
    echo "=== Phase 2b verification run ==="
...
echo "Starting harness HTTP server (OPA authoritative)..."
(
...
echo "Attribution confirmed: pid ${HARNESS_PID} owns ${HARNESS_URL}, OPA reachable (authoritative)"
...
# ── 9. Phase 2b negative test: harness REFUSES to start when OPA is down ──
echo ""
echo "── Negative test: harness must fail when OPA is unreachable ──"
# Stop the harness first
echo "Stopping current harness..."
kill -TERM "${HARNESS_PID}" 2>/dev/null || true
...
# Stop OPA
echo "Stopping OPA sidecar..."
docker stop dazense_opa > /dev/null 2>&1 || true
sleep 2

# Try to start harness — it should FAIL (exit non-zero or never become ready)
NEGATIVE_LOG="${EVIDENCE_DIR}/negative-opa-down.log"
(
    cd "${REPO_ROOT}/harness"
    HARNESS_TRANSPORT=http \
    HARNESS_ALLOW_INSECURE_CONFIG_ONLY=true \
...
for i in $(seq 1 15); do
    if ! kill -0 "${NEG_PID}" 2>/dev/null; then
        ...
        if grep -qi "OPA.*unreachable\|OPA.*health.*FAIL" "${NEGATIVE_LOG}"; then
...
# Kill if still alive (means it didn't fail fast — bad)
if kill -0 "${NEG_PID}" 2>/dev/null; then
    kill -TERM "${NEG_PID}" 2>/dev/null || true
    sleep 1
    kill_listener_on_port
fi

if [ "${NEGATIVE_PASSED}" = true ]; then
    echo "  ✓ harness refused to start with OPA down (fail-fast confirmed)"
else
    echo "  ✗ harness did NOT fail when OPA was down"
    tail -20 "${NEGATIVE_LOG}"
    # Restart OPA before exiting
    docker start dazense_opa > /dev/null 2>&1 || true
    exit 1
fi

# Restart OPA for cleanup
echo "Restarting OPA sidecar..."
docker start dazense_opa > /dev/null 2>&1 || true
sleep 3
...
# ── 11. Summary ──
{
    echo "# Phase 2b Verification — ${TIMESTAMP}"
    echo ""
    echo "## Gates"
    echo "- [x] OPA sidecar reachable, bundle loaded"
    echo "- [x] Harness boots with OPA authoritative (no shadow mode, no in-code rules)"
    echo "- [x] test-query.ts regression passed"
    echo "- [x] test-auth.ts regression passed"
    echo "- [x] test-opa-equivalence.ts 28/28 assertions passed (OPA sole engine)"
    echo "- [x] Negative: harness refuses to start when OPA is down"
    echo ""
...
echo ""
echo "PASS - Phase 2b verification complete. Evidence: ${EVIDENCE_DIR}"
