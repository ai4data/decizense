...
async function main() {
	...
	await runWithRootSpan('dazense-test-auth', 'test_auth.main', { 'dazense.test.name': 'test-auth' }, async () => {
		...
		const findingResult = (await harness.writeFinding(
			'test-auth-002',
			'Test finding for auth verification',
			'high',
			['flights'],
		)) as Record<string, unknown>;
