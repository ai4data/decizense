...
async function main() {
	...
	const init = (await harness.initializeAgent('flight_ops', 'test-001', 'test')) as Record<string, unknown>;
	...
	// Query delayed flights (should work)
	...
	const result2 = (await harness.queryData(
		'flight_ops',
	...
	const result3 = (await harness.queryData(
		'flight_ops',
		'SELECT booking_id, status FROM bookings LIMIT 5',
	)) as Record<string, unknown>;
