...
- Remove `SESSION_ID = session-${Date.now()}` entirely
- **Interactive mode (default)**: require `workflow_id` as a mandatory CLI arg (or equivalent input field); fail fast with a clear error if missing — no derivation fallback, per the canonical rule
- **Batch mode** (`HARNESS_MODE=batch`): accept `workflow_id` OR derive from `sha256(caller_subject + question + intent_version)[0:32]` only if `intent_version` is also provided; fail otherwise
- `session_id` is derived from the resolved `workflow_id`, never from `Date.now()`
...
| 6   | Medium   | Conflicting workflow_id rules (hash of session_id+question, WORKFLOW_ID env, `Date.now()` session)                                | Established a canonical rule in Phase 1b (later refined in Revision 2, finding R2-4). `session_id` derives from `workflow_id`, not the other way around. See the current authoritative rule in Phase 1b "Canonical workflow_id rule" for the final text — this row describes the intermediate Revision 1 state only. |
...
3. **Workflow ID policy locked down** — this guardrail was initially satisfied by a "permanent idempotency" fallback, which was later found (Revision 2, R2-4) to be a correctness hazard for interactive queries. The **current policy** requires callers to provide `workflow_id` explicitly for interactive queries; implicit derivation is permitted only in batch mode and only when an `intent_version` (freshness dimension) is supplied. See the canonical rule in Phase 1b for the authoritative text. The original day-boundary concern is now moot because freshness comes from `intent_version`, not time windows.
