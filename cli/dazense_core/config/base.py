...
class DazenseConfigError(Exception):
    """Raised when dazense config loading fails."""

    pass


class DazenseConfig(BaseModel):
    """dazense project configuration."""

    project_name: str = Field(description="The name of the dazense project")
    ...
    @classmethod
    def promptConfig(cls, project_name: str, existing: "DazenseConfig | None" = None) -> "DazenseConfig":
        """Interactively prompt the user for all dazense configuration options.
    ...
    @classmethod
    def _prompt_extend(cls, existing: "DazenseConfig") -> "DazenseConfig":
    ...
    def save(self, path: Path) -> None:
        ...
        config_file = path / "dazense_config.yaml"
    ...
    @classmethod
    def load(cls, path: Path) -> "DazenseConfig":
    ...
    def try_load(
        ...
        *,
        ...
        Args:
            path: Directory containing dazense_config.yaml. Defaults to DAZENSE_DEFAULT_PROJECT_PATH
            ...
            raise_on_error: If True, raises DazenseConfigError on failure.
        Returns:
            DazenseConfig if loaded successfully, None if failed and both flags are False.
        """
        if path is None:
            default_path = os.environ.get("DAZENSE_DEFAULT_PROJECT_PATH")
        ...
        def handle_error(message: str) -> None:
            if raise_on_error:
                raise DazenseConfigError(message)
            if exit_on_error:
        ...
        if not config_file.exists():
            handle_error("No dazense_config.yaml found in current directory")
            return None
        ...
        try:
            ...
            handle_error(f"Failed to load dazense_config.yaml: Invalid YAML syntax: {e}")
            return None
        except ValidationError as e:
            errors = "; ".join(
                f"{' → '.join(str(x) for x in err['loc']) or 'config'}: {err['msg']}" for err in e.errors()
            )
            handle_error(f"Failed to load dazense_config.yaml: {errors}")
            return None
        except ValueError as e:
            handle_error(f"Failed to load dazense_config.yaml: {e}")
            return None
