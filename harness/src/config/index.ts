...
export class ScenarioLoader {
  ...
  get businessRules(): BusinessRule[] {
    const data = loadYaml<{ rules: BusinessRule[] }>(
      join(this.scenarioPath, "semantics", "business_rules.yml")
    );
    return data.rules;
  }

  get semanticModel(): SemanticModel {
    return loadYaml<SemanticModel>(
      join(this.scenarioPath, "semantics", "semantic_model.yml")
    );
  }
